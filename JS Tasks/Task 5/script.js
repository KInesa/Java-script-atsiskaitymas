...
const users = [
 ...
 function filterDogOwners(){
  let result = users.filter(users.hasDog ===true);
  let names = result.map(users => users.name).join(', ');
  return names
 };
 console.log(filterDogOwners())

 function filterAdults(){
  let result = users.filter(users.age >= 18);
  let names = result.map(users => users.age).join(', ');
  return names
 };
